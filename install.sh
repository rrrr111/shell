...
# export DAT_PATH='/usr/local/share/xray2'
DAT_PATH=${DAT_PATH:-/usr/local/share/xray2}
...
if [[ -f '/etc/systemd/system/xray2.service' ]] && [[ -f '/usr/local/bin/xray2' ]]; then
...
check_install_user() {
  if [[ -z "$INSTALL_USER" ]]; then
    if [[ -f '/usr/local/bin/xray2' ]]; then
      INSTALL_USER="$(grep '^[ '$'\t]*User[ '$'\t]*=' /etc/systemd/system/xray2.service | tail -n 1 | awk -F = '{print $2}' | awk '{print $1}')"
      if [[ -z "$INSTALL_USER" ]]; then
        INSTALL_USER='root'
      fi
    else
...
get_current_version() {
  ...
  if [[ -f '/usr/local/bin/xray2' ]]; then
    CURRENT_VERSION="$(/usr/local/bin/xray2 -version | awk 'NR==1 {print $2}')"
...
install_startup_service_file() {
  mkdir -p '/etc/systemd/system/xray2.service.d'
...
cat > /etc/systemd/system/xray2.service << EOF
[Unit]
...
ExecStart=/usr/local/bin/xray2 run -config /home/xray/config.json
Restart=on-failure
...
${temp_AmbientCapabilities}
${temp_NoNewPrivileges}
ExecStart=/usr/local/bin/xray2 run -config /home/xray/%i.json
Restart=on-failure
RestartPreventExitStatus=23
LimitNPROC=10000
LimitNOFILE=1000000
...
WantedBy=multi-user.target
EOF
  chmod 644 /etc/systemd/system/xray2.service /etc/systemd/system/xray@.service
  if [[ -n "$JSONS_PATH" ]]; then
    "rm" '/etc/systemd/system/xray2.service.d/10-donot_touch_single_conf.conf' \
      '/etc/systemd/system/xray@.service.d/10-donot_touch_single_conf.conf'
    echo "# In case you have a good reason to do so, duplicate this file in the same directory and make your customizes there.
# Or all changes you made will be lost!  # Refer: https://www.freedesktop.org/software/systemd/man/systemd.unit.html
[Service]
ExecStart=
ExecStart=/usr/local/bin/xray2 run -confdir $JSONS_PATH" |
      tee '/etc/systemd/system/xray2.service.d/10-donot_touch_multi_conf.conf' > \
        '/etc/systemd/system/xray@.service.d/10-donot_touch_multi_conf.conf'
  else
    "rm" '/etc/systemd/system/xray2.service.d/10-donot_touch_multi_conf.conf' \
      '/etc/systemd/system/xray@.service.d/10-donot_touch_multi_conf.conf'
    echo "# In case you have a good reason to do so, duplicate this file in the same directory and make your customizes there.
# Or all changes you made will be lost!  # Refer: https://www.freedesktop.org/software/systemd/man/systemd.unit.html
[Service]
ExecStart=
ExecStart=/usr/local/bin/xray2 run -config ${JSON_PATH}/config.json" > \
      '/etc/systemd/system/xray2.service.d/10-donot_touch_single_conf.conf'
    echo "# In case you have a good reason to do so, duplicate this file in the same directory and make your customizes there.
# Or all changes you made will be lost!  # Refer: https://www.freedesktop.org/software/systemd/man/systemd.unit.html
[Service]
ExecStart=
ExecStart=/usr/local/bin/xray2 run -config ${JSON_PATH}/%i.json" > \
      '/etc/systemd/system/xray@.service.d/10-donot_touch_single_conf.conf'
  fi
  ...
  systemd_cat_config /etc/systemd/system/xray2.service
...
start_xray() {
  if [[ -f '/etc/systemd/system/xray2.service' ]]; then
...
remove_xray() {
  if systemctl list-unit-files | grep -qw 'xray'; then
    ...
    local delete_files=('/usr/local/bin/xray2' '/etc/systemd/system/xray2.service' '/etc/systemd/system/xray@.service' '/etc/systemd/system/xray2.service.d' '/etc/systemd/system/xray@.service.d')
...
main() {
  ...
  install_xray
  [[ "$N_UP_SERVICE" -eq '1' && -f '/etc/systemd/system/xray2.service' ]] || install_startup_service_file
  echo 'installed: /usr/local/bin/xray2'
  ...
  if [[ "$SYSTEMD" -eq '1' ]]; then
    echo 'installed: /etc/systemd/system/xray2.service'
